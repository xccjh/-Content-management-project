	...
	// 判断是get请求
	 if(empty($_POST)){  // 如果是get 请求的话,$_POST这个超全局变量里面一定为空
		if(!isset($_GET['action'])){ // 这个判断里面表示跳转过来的页面渲染
			...
			$id = $_GET['id'];
			// 2. 调用方法删除数据
			$res = del('delete from week where id =' .$id);
			// 3. 判断删除的结果
			if($res){
				$arr2 = array(
					'code'=>200,
			...
			$id = $_GET['id'];
			// 2. 查询数据
			$res = query('select * from week where id = ' . $id);
			// 3. 判断查询结果
			if(!empty($res)){
				$arr3 = array(
					'code' => 200,
			...
			$ids = $_GET['ids'];
			// 2. 调用方法进行批量删除
			$sql ='delete from week where id in ('.implode(',',$ids).')';
			$res = del($sql);
			// 3. 判断是否删除成功
			if($res){
				$arr5 = array(
